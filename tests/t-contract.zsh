#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-openshift-aliases — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-openshift-aliases.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-openshift-aliases.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-openshift-aliases.plugin.zsh"
    assert $state equals 0
}
